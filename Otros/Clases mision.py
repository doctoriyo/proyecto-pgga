...
class metodos():
    ...
    def actualiza(self, i):
        ...
        if Mision.hay_mision == True:
            
            self.v_act = Mision.vel_obj
...
class mision():
    def __init__(self, g_part, t, stop):
        
        self.i = 0
        self.k = 1
        self.formacion = False
        self.radio = 5
        self.eps_ang = 0.4
        self.eps_obj = 0.5
        self.pos = False
        self.ang = False
        self.x_obj = float(input("Introduzca x objetivo: "))
        self.y_obj = float(input("Introduzca y objetivo: "))
        self.pos_objetivo = np.array([self.x_obj, self.y_obj])
        self.vel_obj = np.zeros(2)
        self.hay_mision = False
        self.meta = False
    ...
    def together(self):
        ...
        for i in range(n):
            ...
            for j in range(n):
                ...
                self.ang = np.abs(g_part[i].angulo - g_part[j].angulo) < self.eps_ang
                self.pos = norm(np.array(g_part[i].posicion) - np.array(g_part[j].posicion)) < self.radio
                               
                print(self.ang, self.pos)
            
                self.k = self.k * (self.ang*self.pos)
                              
                print(self.k)
            
            """
            Tengo que modificar este bucle porque así solo miran a los de al lado 
            y no entre todos
        ...
        print(self.formacion)
            
            
    def objetivo(self, i):
        
        print(self.pos_objetivo - np.array(g_part[i].posicion))
        
        self.vel_obj = (self.pos_objetivo - np.array(g_part[i].posicion))/norm(self.pos_objetivo - np.array(g_part[i].posicion))
        
        if i == 0:
            
            self.meta = 0
        
        if norm(abs(np.array(g_part[i].posicion)-self.pos_objetivo)) < self.eps_obj:
    ...
    def movimiento(self, t):
        ...
        if t == 0:
            ...
            if self.formacion == True:
                
                self.hay_mision = True
